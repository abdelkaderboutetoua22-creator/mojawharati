...
function buildVersion() {
  ...
  const explicit = getEnv('NEXT_PUBLIC_BUILD_VERSION');
  if (explicit) return explicit;

  // Cloudflare Pages provides these at build time
  const sha = process.env.CF_PAGES_COMMIT_SHA || process.env.CF_PAGES_BRANCH || '';
  const dep = process.env.CF_PAGES_DEPLOYMENT_ID || '';
  const shortSha = sha && sha.length >= 7 ? sha.slice(0, 7) : sha;

  const ts = new Date().toISOString().replace('T', ' ').replace('Z', ' UTC');
  if (shortSha) return `cf:${shortSha}${dep ? `/${dep.slice(0, 8)}` : ''} @ ${ts}`;
  return `dev @ ${ts}`;
}
